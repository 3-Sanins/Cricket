...
def choose(leader,team_list,score,wicket,oppo,fpl,scoreboard,balls,six,fours,t1,t2):
    ...
    for i in team_list:
        Button(choosing,text=i,command=lambda:bat(leader,team_list,score,wicket,i,oppo,fpl,choosing,scoreboard,balls,six,fours,t1,t2)).pack()
...
def bat(leader,team_list,score,wicket,pl,oppo,fpl,choosing,scoreboard,balls,six,fours,t1,t2):
    ...
    #messagebox.showinfo("Choice",index)
    choosing.destroy()
...
def details():
    ...
    Button(detail,text="Submit",command=lambda:dsubmit(p1,p2,detail)).grid(row=3,column=0,columnspan=2,padx=5)
    detail.pack()

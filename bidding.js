...
// Display current player (always index 0)
...
function transferPlayerToTeam(player, receivingUser, callback) {
  ...
  teamRef.once('value', (snapshot) => {
    ...
    teamRef.set(team).then(() => {
      ...
      players.splice(0, 1); // Remove index 0
...
document.getElementById('take-btn').addEventListener('click', () => {
  ...
  if (players.length > 0 && userData.money !== undefined) {
    ...
    otherUserRef.once('value', (snapshot) => {
      ...
      const currentPrice = parseFloat(players[0].price);
      ...
      const playerRef = database.ref('players/0'); // Update index 0
  ...
  if (players.length > 0) {
    ...
    if (bidder === "" || bidder === "None") {
      ...
      userBidRef.set(1).then(() => {
        alert('You have left the player.');
